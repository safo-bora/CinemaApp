...
class LocalReplicator: ReplicatorProtocol {
    ...
    */
    func readFile() -> Any? {
        let dataSourceFilename: String = "debug"
    ...
    func processData(_ jsonResult: AnyObject?) {
        ...
        if let eventList = jsonResult {
            for index in 0..<eventList.count {
                ...
                eventItem[MovieAttributes.title.rawValue] = "[LOCAL] \(eventItem[MovieAttributes.title.rawValue]!)"  as AnyObject?

                //Generate event UUID
                eventItem[MovieAttributes.eventId.rawValue] = UUID().uuidString as AnyObject?

                //Generate semi random generated attendeeslist
                eventItem[MovieAttributes.attendees.rawValue] = ActorsGenerator.getSemiRandomGeneratedActorsList() as AnyObject?
