...
class RemoteReplicator: ReplicatorProtocol {
    ...
    */
    func fetchData() {
        ...
        //Remote resource
        let request: URLRequest = URLRequest(url: URL(string: "https://www.dropbox.com/s/0qy5ocjgg2l8zag/katya_json5.json?dl=1")!)
    ...
    internal func processData(_ jsonResponse: AnyObject?) {
        ...
        if let eventList = jsonResult {
            for index in 0..<eventList.count {
                ...
                eventItem[MovieAttributes.title.rawValue] = (eventItem[MovieAttributes.title.rawValue]!) as AnyObject?

                //Generate event UUID
                eventItem[MovieAttributes.eventId.rawValue] = UUID().uuidString as AnyObject?

                //Generate semi random generated attendeeslist
                eventItem[MovieAttributes.actors.rawValue] = ActorsGenerator.getSemiRandomGeneratedActorsList() as AnyObject?
